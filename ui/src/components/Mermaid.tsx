...
const mermaidConfig: MermaidConfig = {
  ...
  `,
  fontFamily: 'var(--font-geist-sans, inherit), var(--font-serif-jp, inherit), sans-serif',
  fontSize: 12,
};
const ensureMermaidInit = (): void => {
...
const DEFAULT_LABELS: MermaidLabels = {
  ...
  syntaxError: '図表に構文エラーがあり、レンダリングできません。',
  rendering: '図表を描画中...',
};
...
const FullScreenModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 p-4">
      <div
        ref={modalRef}
        className="bg-[var(--card-bg)] rounded-lg shadow-xl max-w-5xl max-h-[90vh] w-full overflow-hidden flex flex-col"
      >
        {/* Modal header with controls */}
        <div className="flex items-center justify-between p-4 border-b border-[var(--border-color)]">
          <div className="font-medium text-[var(--foreground)] font-serif">{title}</div>
...
const Mermaid: React.FC<MermaidProps> = ({ chart, className = '', zoomingEnabled = false, labels: userLabels = {} }) => {
        ...
        if (isMounted) {
          ...
          if (mermaidRef.current) {
            mermaidRef.current.innerHTML = `
              ...
              <pre class="text-xs overflow-auto p-2 bg-[var(--card-bg)] rounded">${chart}</pre>
            `;
          }
        }
      }
