"""Define the current SQLite schema and Git ref layout of an archive pair.

The format covers immutable observations, sync recovery, independent maintenance
jobs, offline-reader indexes, and permanent names for retained Git evidence.
"""
...
_REF_PREFIX = "refs/github-archive"
...
def pull_ref(number: int, role: str, sha: str) -> str:
    ...
    return f"{_REF_PREFIX}/pulls/{number}/{role}/{sha}"
...
def pull_staging_ref(number: int) -> str:
    ...
    return f"{_REF_PREFIX}/staging/pulls/{number}/head"
...
def upstream_ref(kind: str, sha: str) -> str:
    ...
    return f"{_REF_PREFIX}/upstream/{kind}/{sha}"
...
def commit_ref(sha: str) -> str:
    ...
    return f"{_REF_PREFIX}/commits/{sha}"
...
def source_staging_ref(source: str) -> str:
    ...
    return f"{_REF_PREFIX}/staging/sources/{digest}"
