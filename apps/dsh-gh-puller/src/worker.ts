...
const defaultSpawn: Spawner = (args, cwd) => spawn('uv', args, { cwd, stdio: ['pipe', 'pipe', 'pipe'] })

type Finish = (result: WorkerResponse | Error) => void

export class WorkerClient {
  private child: ChildProcess | null = null
  private buffer = ''
  private nextId = 1
  private pending = new Map<number, Finish>()
  private disposed = false

  constructor(
    private readonly serverDir: string,
    private readonly spawner: Spawner = defaultSpawn,
  ) {}
  ...
  async request(action: string, params: Record<string, unknown> = {}, signal?: AbortSignal): Promise<string> {
    let last: unknown
  ...
  private dispatch(action: string, params: Record<string, unknown>, signal?: AbortSignal): Promise<WorkerResponse> {
    if (signal?.aborted) return Promise.reject(new WorkerError('aborted'))
  ...
  private ensure(): ChildProcess {
    if (this.disposed) throw new WorkerError('worker disposed')
  ...
  private onData(chunk: string): void {
    ...
    for (;;) {
      ...
      try {
        ...
        if (typeof resp.id === 'number') this.pending.get(resp.id)?.(resp)
      } catch {
  ...
  private flushPending(error: WorkerError): void {
