"""Test hub state/protocol behavior and HTTP routes without network egress.
...
class _FakeWs:
    """Collect text frames emitted by hub protocol functions."""
...
def test_subscribe_live_and_finalize():
    """Two subscribers receive ordered live events and the finalized state."""
    hub = _Hub()
    viewers = [_FakeWs(), _FakeWs()]

    async def scenario() -> None:
        await hub.feed(_evt("session/start", "s1", seq=0, run_id="chat:demo", label="chat:demo"))
        await hub.feed(_evt("turn/start", "s1", seq=1, turn=1))
        await hub.feed(_evt("step/start", "s1", seq=2, turn=1, step=1))
        await hub.feed(_user_evt("s1", 3, "hi"))
        for viewer in viewers:
            await _viewer_frame(viewer, hub, {"type": "subscribe", "session": "s1"})
        await hub.feed(_evt("assistant/chunk", "s1", seq=4, turn=1, step=1,
                            chunk={"type": "text", "index": 0, "text": "世"}))
        await hub.feed(_evt("session/end", "s1", seq=5, state="completed", ok=True))

    asyncio.run(scenario())
    for viewer in viewers:
        assert viewer.sent[0] == {"type": "evt_ready", "session": "s1", "lastSeq": 3}
        assert [frame["event"]["seq"] for frame in viewer.sent[1:]] == [4, 5]
    assert hub.index()[0]["state"] == "completed"


def test_viewer_history_pagination_missing_session_and_ping():
    hub = _Hub()
    viewer = _FakeWs()

    async def scenario() -> None:
        for i in range(10):
            await hub.feed(_user_evt("page1", i, str(i)))
        await _viewer_frame(viewer, hub, {"type": "history", "session": "page1", "max": 5})
        await _viewer_frame(viewer, hub, {
            "type": "history", "session": "page1", "beforeSeq": 5, "max": 5,
        })
        await _viewer_frame(viewer, hub, {"type": "history", "session": "ghost"})
        await _viewer_frame(viewer, hub, {"type": "ping"})

    asyncio.run(scenario())
    page, older, missing, pong = viewer.sent
    assert [event["seq"] for event in page["events"]] == [5, 6, 7, 8, 9]
    assert page["hasMore"] is True and page["nextBeforeSeq"] == 5
    assert [event["seq"] for event in older["events"]] == [0, 1, 2, 3, 4]
    assert older["hasMore"] is False and older["nextBeforeSeq"] is None
...
def test_history_merges_disk_and_memory(tmp_path):
    ...
    viewer = _FakeWs()
    asyncio.run(_viewer_frame(viewer, hub, {"type": "history", "session": "merge1"}))
    assert [event["seq"] for event in viewer.sent[0]["events"]] == [0, 1, 2]
...
def test_get_serves_viewer_html():
    ...
    async def scenario() -> None:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            assert "文件缺失" in (await client.get("/viewer")).text
            assert (await client.get("/no-such-path")).status_code == 404
...
def test_default_static_serves_viewer(tmp_path):
    ...
    async def scenario() -> None:
        transport = httpx.ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            assert "<html>viewer-pg</html>" in (await client.get("/")).text
...
def test_disk_seed_index_and_history(tmp_path):
    """重启种子(事件溯源格式,扁平):索引含 run_id/state,历史可回放。
    ...
    viewer = _FakeWs()
    asyncio.run(_viewer_frame(viewer, hub, {
        "type": "history", "session": "judge:llm/seed-a",
    }))
    page = viewer.sent[0]
    assert [event["type"] for event in page["events"]] == [
        "session/start", "user/message", "assistant/message", "session/end",
    ]
...
def test_delete_removes_memory_and_disk_and_broadcasts(tmp_path):
    """Delete removes both stores and broadcasts the remaining index."""
    ...
    async def scenario() -> None:
        await hub.feed(_evt("session/start", "live1", seq=0, run_id="chat:x", label="chat:x"))
        viewer.sent.clear()
        await _viewer_frame(viewer, hub, {"type": "delete", "session": "gh-puller/dsk"})

    asyncio.run(scenario())
    assert {session["session"] for session in viewer.sent[-1]["sessions"]} == {"live1"}
...
def test_feed_batch_forwards_once_and_keeps_chunks_out_of_history(monkeypatch):
    ...
    subscriber = _FakeWs()
    hub.sessions["batch"].subscribers.add(subscriber)
    ...
    assert subscriber.sent == [{"type": "evts", "events": [chunk, message]}]
