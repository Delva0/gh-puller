...
- `WS /ws` — 一连接一角色,首帧定角色:`{"type":"evts","events":[...]}` 为生产端(事件接入),
...
hub 只持内存状态(每会话非流式历史 + 原始流高水位),`assistant/chunk` 仅实时批量
转发,不会驻留为历史;写盘是 FileSink 的事。磁盘布局为
扁平 `<uuid>.jsonl`(隐式分类学:会话键 = 事件内 `session/start` 的
`session` 字段,状态 = 有无 `session/end` 行),启动时从 `AGENT_MONITOR_DIR`
(见 `gh_puller/envs.py`)种子历史,重启 hub
...
- 会话运行期,生产端每 `AGENT_MONITOR_HEARTBEAT_SECS`(默认 30s)触碰 JSONL mtime,
  不向事件流写入心跳事件;
...
注意 WS 地址带 `/ws` 路径。协议帧格式测试见 `tests/test_hub.py`
(`uv --directory apps/agent-monitor/server run pytest`)。
