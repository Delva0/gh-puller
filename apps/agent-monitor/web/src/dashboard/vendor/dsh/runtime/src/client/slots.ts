 ...
 * redundancy. */
import { SlotCore } from '@dsh/ui-slots'
import type {
  HostObservable, LiveSlotNode, LocaleFace, OwnerOf, SessionMaybeProvideInfo, SlotEntryDef,
  SlotMap, SlotRenderer, SlotRendererHost, SlotScope, SlotSpec, StoreDecl, StoreFactory, StoredEntry,
  StoreInstanceLike,
} from '@dsh/ui-slots'
...
/** Cordis-efit 生命周期缩减(gh-puller 内嵌):同步运行回调并保存其释放函数/释放函数组。
 *
 * dsh 中每个 `ctx.effect(callback, label)` 属于调用者 fiber(插件卸载级联);
 * 本宿主单装配 + DS 单 app 生命周期,不做 fiber 级联——保留"立即生效、释放函数
 * 幂等可逆"的核心语义。
 */
function runEffect(callback: () => unknown, label: string): () => void {
  void label
  ...
  if (typeof result === 'function') {
    ...
    for (const d of result as Iterable<() => void>) disposers.push(d)
  }
  let disposed = false
  return () => {
    if (disposed) return
...
export class SlotRegistry {
  ...
  /** hostFace 的 sessions/workspaces 面(桥接层经 setHostFaces 注入;未注入时 renderSlot('root') 抛错,同 dsh 次序错误语义)。 */
  private _faces: { sessions?: unknown; workspaces?: unknown; provided?: boolean } = {}

  /**
   * @param ctx - Cordis 根上下文(已弃用;保留参数签名兼容桥接层调用点)。
   */
  constructor(_ctx?: unknown) {
    void _ctx
    // 原 ctx.emit('slots/changed') 无监听者;仅保留变更钩子占位。
    this._core.onMutate(() => {})
  }

  /** 注入 hostFace 所需 faces(桥接层使用;非 'root' 渲染流程无需调用)。 */
  setHostFaces(faces: { sessions: unknown; workspaces: unknown }): void {
    this._faces.sessions = faces.sessions
    this._faces.workspaces = faces.workspaces
    this._faces.provided = true
  }

  /** 渲染器 host 出口(桥接层 renderRoot 用;等价 dsh 内部 hostFace)。 */
  getRendererHost(): SlotRendererHost {
  ...
  inject(key: keyof SlotMap & string, callback: () => SlotInjectionEffect): () => void {
    // 去 Cordis:外层 effect 缩减为同步运行(生命周期 = 单装配寿命)。
    const disposeController = runEffect(() => {
      ...
      const reconcile = (): void => {
        ...
        // A declaration lifetime 原为嵌套 Cordis effect;此处改为同步运行。
        const disposeEffect = runEffect(callback, `slots.inject(${JSON.stringify(key)}): declaration`)
  ...
  private _register(options: ErasedRegisterOptions, component: unknown): () => void {
    ...
    const registrant = options.registrant // 原 this.ctx.fiber(注册者 fiber)不可用,站名为 null
  ...
  private hostFace(): SlotRendererHost {
    ...
    // 去 Cordis:faces 由桥接层 setHostFaces 注入(本宿主不渲染 'root',常规路径不触发)。
    if (!this._faces.provided) {
      throw new Error("renderSlot('root') before the host faces injected — boot order puts runtime apply first")
    }
    const sessions = this._faces.sessions as {
      list: HostObservable<unknown>
      currentProvideInfo: HostObservable<SessionMaybeProvideInfo>
    }
    const workspaces = this._faces.workspaces as { list: HostObservable<unknown> }
    ...
    return host!  // 上方已赋值(faces 注入检查在前)
  }
...
;(SlotRegistry.prototype as { register: (options: object, component: unknown) => () => void }).register
  = function register(this: SlotRegistry, rawOptions: object, component: unknown): () => void {
    ...
    return runEffect(() => this['_register'](options, component), 'slots.register()')
  }
