...
- 引擎（纯数据 dataclass/dict + 纯函数式生成协议、缓存与状态 IO，**零 pydantic、零 Request 概念、零
  ...
  在本目录 `app.py` / `schemas.py`;graphify 组装（图服务/建图/MCP 工具桌 + runtime_config 覆盖构造参数注入）
  唯一收容点在本目录 `generators.py`
...
- target 契约:generator + generator_config(cc/dsh/codex = 本地配置文件路径 `config_path`,
  服务端纯透传给 agent SDK;llm = provider/model/凭证 dict),见 `gh_puller/agent/generators:GENERATORS`;
  graphify MCP 配置与图服务由本目录 `generators.py` 经 `runtime_config` 注入 generator_config
  (引擎 adapter 只做白名单透传)
