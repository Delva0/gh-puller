...
export default function ProcessedProjects({
  ...
  return (
    <div className={`${className}`}>
      {showHeader && (
        <header className="mb-6">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold text-[var(--accent-primary)]">{t('projects.title')}</h1>
      ...
      {isLoading && <p className="text-[var(--muted)]">{t('projects.loadingProjects')}</p>}
      {error && <p className="text-[var(--highlight)]">{t('projects.errorLoading')} {error}</p>}
              ...
              <div key={project.id} className="relative p-4 border border-[var(--border-color)] rounded-lg bg-[var(--card-bg)] shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.02]">
                ...
                >
                  ...
                  <p className="text-xs text-[var(--muted)]">
                    {t('projects.processedOn')} {new Date(project.submittedAt).toLocaleDateString()}
                  </p>
                </Link>
              </div>
            ) : (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[var(--muted)] mt-1">
                      {t('projects.processedOn')} {new Date(project.submittedAt).toLocaleDateString()} • {project.repo_type} • {project.language}
                    </p>
                  </div>
      ...
      {!isLoading && !error && projects.length > 0 && filteredProjects.length === 0 && searchQuery && (
        <p className="text-[var(--muted)]">{t('projects.noSearchResults')}</p>
      )}

      {!isLoading && !error && projects.length === 0 && (
        <p className="text-[var(--muted)]">{t('projects.noProjects')}</p>
      )}
    </div>
  );
}
