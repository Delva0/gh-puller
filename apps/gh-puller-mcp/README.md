...
后端维护两条随本服务存活、彼此隔离的原生 MCP frontend：

```
                                      ┌─ persistent query frontend ────┐
MCP client ──stdio/HTTP──> gh-puller-mcp                              ├─IPC──> shared daemon
                                      └─ lazy indexing frontend ──────┘
```

服务启动并初始化查询 frontend，随后在它的 newline-framed MCP 流上发送普通 `tools/call`。
第一次调用 `index_repository` 时按需初始化独立的 indexing frontend，避免长时间建图占住查询流；
两条流各自串行，均原样转发原生 `CallToolResult` 信封(content / structuredContent / isError)。
两个 frontend 复用同一个 CBM shared daemon，并以持续会话阻止 session-managed daemon 在相邻请求
之间退出；服务关闭时 EOF 释放会话。CBM 自带的 `cli` 是 one-shot daemon client：每个进程都会
连接 daemon，无其他会话时还会启动一个临时 daemon，完成后立即断开，因此不用于服务热路径。
...
* stdio 模式在第一次合法工具调用时启动查询 frontend，随后复用至 EOF；HTTP 模式在监听端口前完成查询 frontend 初始化，因此请求不承担 CBM 进程、build fingerprint 或 daemon 冷启动成本。indexing frontend 在第一次 `index_repository` 时按需启动，但复用已存活的 shared daemon。
* 工具请求不自动重试；超时或后端退出会结束对应的 frontend，下一次调用可建立新会话。
...
* HTTP 请求无调用方会话，但后端有服务级持续会话。工具调用在线程中等待后端，uvicorn event loop 仍可响应 `ping`、`tools/list` 和其他连接；普通工具在查询 frontend 中按序执行，`index_repository` 在独立 frontend 中执行，二者互不占用对方的请求流。
...
* 两条持久后端都是普通的 C frontend 会话，因此沿用 CBM daemon 的 session context、auto-watch 和后台任务语义；本服务不提供 C UI / daemon 控制面(`--port` 9749 UI 不在范围内，`--http` 暴露的是 Streamable HTTP MCP)。外层 SDK 的 `notifications/cancelled` 仍是 no-op。
