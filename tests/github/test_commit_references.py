...
def test_observation_commit_references_preserve_every_source_path() -> None:
    shas = [f"{number:040x}" for number in range(1, 8)]
    references = (
        *observation_commit_references(
            "pull-commits",
            {"value": [{"sha": shas[0], "tree": {"sha": f"{9:040x}"}}]},
        ),
        *observation_commit_references(
            "pull-reviews",
            {"value": [{"id": 71, "commit_id": shas[1]}]},
        ),
        *observation_commit_references(
    ...
    assert references[0].field_path == "/value/0/sha"
